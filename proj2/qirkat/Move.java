...
class Move {
     ...
     * O to destination D.
     */
    static Move move(int o, int d) {
        return move(col(o), row(o), col(d), row(d));
    }

    /**
     * Return a vestigial Move consisting only of starting square
     * O do destination D given NEXTJUMP.
     */
    static Move move(int o, int d, Move nextJump) {
        return move(col(o), row(o), col(d), row(d), nextJump);
    }
    ...
    static Move move(Move move0, Move move1) {
        ...
        if (move0.jumpTail() == null) {
            ...
            return move(move0, move(move0.jumpTail(), move1));
        }
     ...
     */
    char jumpedRow() {
        return isJump() ? (char) ((row0() + row1()) / 2) : row1();
    }
     ...
     */
    char jumpedCol() {
        return isJump() ? (char) ((col0() + col1()) / 2) : col1();
    }
    ...
    /**
     * Helper method to return string of char C.
     */
    ...
    private void toString(Formatter out) {
        ...
        if (jumpTail() != null) {
            out.format(s(col0()) + s(row0()));
            out.format("-");
            out.format(jumpTail().toString());
        } else {
            out.format(s(col0()) + s(row0()) + "-" + s(col1()) + s(row1()));
        }
    }
    ...
    /**
     * Stores my string representation.
     */
